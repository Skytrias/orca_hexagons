...
style_sizex_full :: proc(style: ^Style, value: f32 = 1, relax: f32 = 0) {		
	style.size.x = { kind = .PARENT, value = value, relax = relax }
	style.mask += { .SIZE_WIDTH }
}

style_sizex :: proc(style: ^Style, value: f32, relax: f32 = 0) {
	style.size.x = { kind = .PIXELS, value = value, relax = relax }
	style.mask += { .SIZE_WIDTH }
}

style_sizemx :: proc(style: ^Style, value: f32, relax: f32 = 0) {
	style.size.x = { kind = .PARENT_MINUS_PIXELS, value = value, relax = relax }
	style.mask += { .SIZE_WIDTH }
}

style_sizey_full :: proc(style: ^Style, value: f32 = 1, relax: f32 = 0) {
	style.size.y = { kind = .PARENT, value = value, relax = relax }
	style.mask += { .SIZE_HEIGHT }
}

style_sizey :: proc(style: ^Style, value: f32, relax: f32 = 0) {
	style.size.y = { kind = .PIXELS, value = value, relax = relax }
	style.mask += { .SIZE_HEIGHT }
}

style_sizemy :: proc(style: ^Style, value: f32, relax: f32 = 0) {
	style.size.y = { kind = .PARENT_MINUS_PIXELS, value = value, relax = relax }
	style.mask += { .SIZE_HEIGHT }
}

style_layout :: proc(style: ^Style, axis: oc.ui_axis, margin: [2]f32 = {}, spacing: f32 = 0) {
...
style_before_on_hover :: proc(ui: ^oc.ui_context, style: Style) {
	...
	oc.ui_pattern_push(&ui.frameArena, &pattern, { 
		kind = .STATUS,
		status = { .HOVER },
	})
